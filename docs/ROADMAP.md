...
> 📧 **Digest diario opcional por correo** ([ADR-0028](DECISIONS.md#adr-0028)): un correo por hogar y miembro al día (`finlia:send-reminder-digests`, 06:30), **solo si hay urgentes** y **solo a quien lo pidió** en `/recordatorios` (opt-in por miembro; jamás marketing). Enviado síncrono por el cron vía SMTP (Brevo free en producción, ver DEPLOYMENT §4); el Mailable no usa cola todavía — el seam es `ShouldQueue` + cola `database`. El correo trae título/fecha/monto de las urgentes y el enlace, nunca saldos ni cuentas.
