...
> ([ADR-0032](DECISIONS.md#adr-0032)) · **05 ✅ eliminación/suspensión ([ADR-0033](DECISIONS.md#adr-0033)) · 06 ✅ política de datos + export ZIP ([ADR-0034](DECISIONS.md#adr-0034))**.
...
> 📝 **Nota de UX (2026-09-19).** El cálculo del hero es correcto y usa la **cuota mensual** de la deuda (`DebtService::dueUntil()` sobre `Debt::monthlyCommitment()`), no el saldo total; pero cuando hay mucho apartado en metas el número −$X del hero no cuadra a simple vista con lo que el usuario ve en el resto del panel: `cash = saldo − apartado en metas − (fijos + recurrentes + cuota de deuda del ciclo + ahorro pendiente)`. Detectado sobre usuario real que interpretó el faltante como un error de la app cuando en realidad tenía casi todo el saldo apartado en metas y varios gastos variables registrados como recurrentes. **Aplicado en el primer pase**: componente `x-liquidity-breakdown` que enseña saldo, apartado en metas y compromisos del ciclo cuando `liquidity.status = short`, en `_hero-enfoque` y en `available-money-card`, y copy retocado para nombrar metas y compromisos. **Pendiente para un segundo pase si no basta**: (1) reemplazar o duplicar el KPI "Deuda total" por "Cuota de deuda este ciclo"; (2) revisar el flujo de alta de recurrentes para orientar hacia presupuestos cuando el gasto es variable (gasolina, mercado).

> 🔧 **Cupo diario estable (2026-09-23, [ADR-0047](DECISIONS.md#adr-0047)).** El "puedes gastar hoy" se congela al inicio del día y muestra el cupo restante: gastar dentro del cupo lo lleva a 0, no baja el promedio. El día siguiente se recompone solo con el saldo real.

> 📦 **Sobres opt-in y KPI de deuda del mes (2026-09-23, [ADR-0048](DECISIONS.md#adr-0048)).** Los presupuestos por categoría ganan una casilla "tratar como sobre": cuando está marcada, la plata del presupuesto se aparta del cupo diario y se descuenta al registrar el gasto en esa categoría. Resuelve el caso lumpy (mercado, gasolina, servicios) sin cambiar el comportamiento de quienes ya usan los presupuestos como metas blandas. En la misma entrega: KPI "Cuota de deuda este mes" en el dashboard, junto a "Deuda total"; y aviso en el formulario de recurrentes sugiriendo un presupuesto con sobre cuando el gasto varía mucho.
