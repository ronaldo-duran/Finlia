...
> 📝 **Nota de UX pendiente (2026-09-19).** El cálculo es correcto y usa la **cuota mensual** de la deuda (`DebtService::dueUntil()` sobre `Debt::monthlyCommitment()`), no el saldo total; pero la tarjeta hero puede leerse rara cuando hay mucho apartado en metas: `cash = saldo − apartado en metas − (fijos + recurrentes + cuota de deuda del ciclo + ahorro pendiente)`. Un hogar con saldo alto pero con casi todo reservado en metas ve un faltante que no encaja con la deuda total que muestra el KPI de al lado, porque el KPI enseña el saldo entero de la deuda y el hero solo cuenta la cuota. Ideas cuando se retome (ninguna urgente): (1) desglose visible en el hero cuando `liquidity.status = short` con los valores que ya devuelve `liquidity` (`current_balance`, `set_aside`, `reserved`); (2) reemplazar o duplicar el KPI "Deuda total" por "Cuota de deuda este ciclo"; (3) matizar el copy del hero para mencionar metas y cuotas explícitamente.
