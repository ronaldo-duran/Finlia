...
- **Invitaciones al hogar** ([ADR-0039](DECISIONS.md#adr-0039)):
  - **Ver** la invitación es público (`GET invitaciones/{token}`, throttle): el token es la autorización. La página muestra solo hogar, rol y caducidad, y es idéntica exista o no una cuenta con ese correo. **Aceptar** sigue tras `auth` + `verified`.
  - El enlace es **transferible** (se reenvía por WhatsApp), así que no prueba el control del buzón: quien se registra desde él **verifica su correo igual**, y la vinculación al hogar ocurre al confirmarlo, nunca antes. El correo del registro lo impone el servidor desde la invitación; el campo del formulario no se usa.
  - El token viaja en la sesión entre la invitación y el login/registro, nunca en la URL.
