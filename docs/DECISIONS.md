...
- [ADR-0046 — Rieles de monetización sin cripples: freemium con grandfather, `ee/` con puerta condicional](#adr-0046) — **ACEPTADA**
...
1. **El veredicto se calcula fuera del modelo.** Claude revisa y escribe **un único archivo** `review.json` con sus hallazgos y una severidad por hallazgo. Un paso posterior de bash —sin modelo— traduce eso a un evento de review de GitHub. Claude no tiene ninguna herramienta de GitHub: no puede aprobar, no puede pedir cambios, no puede comentar. **El único camino al veredicto verde es que el JSON no traiga ningún hallazgo bloqueante.**
2. **Tres resultados, ninguno mergea.** `bloqueante` → `REQUEST_CHANGES` y check rojo; solo hallazgos de tipo `revisar` → `COMMENT` con el aviso de que hay puntos que **necesitan decisión humana**, y check verde; nada → `COMMENT` y check verde. *(Se decidió con `APPROVE` en los dos últimos casos, pero GitHub no permite que Actions apruebe un PR: ver el añadido tras probarlo, al final de este ADR.)* El workflow no tiene `contents: write` y no existe ninguna llamada a merge: **mergear es de una persona, siempre**. El check no cuenta como aprobación y no bloquea el merge salvo que se marque obligatorio en la protección de rama.
...
5. **Sin `--max-turns`; el gasto se acota por tiempo** (`timeout-minutes`) y con la `concurrency` que ya cancela runs viejos. Un tope de turnos bajo produce una revisión truncada, y un veredicto verde emitido sobre una lectura incompleta es peor que no revisar: da falsa tranquilidad.
...
1. **GitHub no permite que Actions apruebe un PR.** El `POST` de la review con evento `APPROVE` devuelve **422** con el mensaje *«GitHub Actions is not permitted to approve pull requests»*, confirmado en el run del 13/09; `REQUEST_CHANGES` sí funciona. Así que la señal verde la da un **check run** propio (`Revisión de Claude`), independiente de la API de reviews, y cuando no hay bloqueantes el veredicto se publica **directamente como `COMMENT` con sus comentarios en línea**. Intentar primero `APPROVE` solo gastaba dos llamadas que fallan siempre y dejaba los hallazgos 🟡 sin su línea. Ese check no bloquea el merge salvo que se marque obligatorio en la protección de rama, lo que mantiene la regla de que decide una persona.
...
10. **Una guía por pantalla, con el nombre de ruta EXACTO.** La primera versión usaba comodines (`debts.*`) creyendo que el detalle de una deuda era «la misma pantalla» que el listado. No lo es: allí no existe ninguno de los anclajes del listado, así que la guía se reducía a un paso huérfano describiendo otra cosa — y de paso la daba por vista, con lo que la de verdad no volvía a salir. Las pantallas de detalle tienen ahora **su propia guía**, y `TourServiceTest` rechaza cualquier comodín y cualquier ruta disputada por dos guías.
...
- **Lo que se esconde al desplazar hay que devolverlo a la vista.** El botón «+» se oculta al bajar la página, y una guía baja la página sola para señalar cada cosa: el paso que hablaba del «+» llegaba justo después de ocultarlo. Se resuelve con `body.tour-abierto`, que suspende ese auto-ocultado mientras la guía está abierta — y **sin transición**, porque el motor mide la opacidad computada para decidir qué pasos valen y con una animación de por medio seguía leyendo 0. Por lo mismo, la marca en el `<body>` se pone **antes** de medir los pasos.
- **Una guía es modal: su fondo captura los clics.** Es deliberado —nadie debe atravesarla sin querer—, pero significa que cualquier recorrido automatizado que empiece con sesión nueva tiene que descartarla antes de tocar la interfaz, igual que haría una persona. Lo descubrió el CI: la guía de bienvenida se comió el clic al avatar en el test de cierre de sesión. El helper `cerrarGuia()` de `tests/e2e/helpers.ts` es la vía, y `auth.setup.ts` la descarta antes de guardar la sesión que reutiliza la suite.
...
## ADR-0046
### Rieles de monetización sin cripples: freemium con grandfather, `ee/` con puerta condicional — **ACEPTADA**

**Contexto.** El MVP está en producción con usuarios reales. La Épica 12 llega para monetizar sin degradar la experiencia gratuita: la premisa es que **Free siga siendo útil de verdad** y que Premium se cobre por funciones que realmente valen la pena, no por reintroducir dolores artificiales. Al mismo tiempo, tres restricciones marcan el diseño:

1. Los usuarios existentes **no pueden romperse**. Un límite nuevo que estrangule un hogar en producción sería una ruptura de contrato.
2. La licencia dual del repo ([ADR-0042](#adr-0042)) exige que borrar `ee/` deje la app funcionando; esto ata cómo se conectan las funciones Premium al núcleo AGPL.
3. Todavía no hay pasarela de pago (llega en v0.41 con Wompi). Los rieles deben soportar activación manual desde `artisan` sin comprometer la seguridad de la puerta.
...
7. **Árbol de decisión de compras como palanca Freemium temprana.** Al registrar un gasto que **queda fuera del presupuesto planeado del mes** (no hay fila en `budgets` para su categoría con el año/mes del gasto), se ofrece un modal de 4 preguntas: previsto, tipo (necesidad/gusto/capricho/emergencia), ánimo (escala 1..5 con iconos Bootstrap del sistema de diseño) y disparador. El azar sale del disparo — la señal es la ausencia de plan, y así los usuarios reciben el árbol donde tiene sentido preguntar. **5 respuestas al mes en Free, ilimitadas en Premium.** La respuesta almacena además un snapshot demográfico —edad derivada de `birth_date` al momento y `gender` del perfil— porque una persona no piensa igual según su edad y esa capa es la que dará valor al modelo de ML. A los 30 días se le pide un **seguimiento** (mismo `mood` posterior + arrepentimiento opcional + nota) en `/compras/revisar`; cerrar la conversación con uno mismo es lo que convierte el árbol en insight y no en encuesta. La data se acumula para futuros insights (feature Premium `compulsive_insights`) y para dataset de ML sin comprometer el flujo normal de registro de gastos. Los usuarios Free ven la app completa; solo pierden profundidad, no funcionalidad.
...
9. **Rieles quietos: "Premium para todos" mientras el catálogo comercial se define.** `SubscriptionService` lee `finlia.subscription.premium_for_all` (encendido por defecto en v0.39); mientras el flag esté encendido, `planFor()` devuelve Premium para cualquier hogar y los topes Free (1 hogar, 2 personas, 5 encuestas) no se aplican. El día que se defina qué lleva cada plan y su precio real, se apaga el flag: los hogares con `finlia:grant-premium` conservan Premium por su suscripción y el resto revierte al comportamiento Free descrito arriba, sin migraciones ni cambios de código. La pantalla `/perfil → Plan` deja de mostrar el CTA "próximamente" y explica en una línea el estado transitorio.
...
- **La encuesta de compras tiene tope Free por defecto (5/mes)**, un valor conservador que se ajusta desde `config/finlia.php` o subiendo el `limits.compulsive_surveys_per_month` en el seeder. Cambiar el tope de un plan existente es un `db:seed` idempotente.

**Estado.** ACEPTADA — 2026-09-19. Implementada en `database/migrations/2026_09_19_00000{1,2,3,10}_*.php`, `app/Enums/{PlanSlug,SubscriptionStatus,PlanFeature,PlanLimit,Compulsive*}.php`, `app/Models/{Plan,Subscription,CompulsiveSurveyResponse}.php`, `app/Services/{SubscriptionService,CompulsiveSurveyService}.php`, `app/Console/Commands/{GrantPremium,RevokePremium}.php`, `app/Http/Controllers/{ProfileController,CompulsiveSurveyController,HouseholdController,ExpenseController}.php`, `ee/src/EeServiceProvider.php`, `resources/views/profile/plan.blade.php`, `resources/views/layouts/partials/compulsive-survey-modal.blade.php` y `tests/Feature/Subscription/*`.
